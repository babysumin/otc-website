...
export default function PolicyPage() {
  ...
  async function fetchPolicy() {
    const { data } = await supabase.from('club_policy').select('content, updated_at').eq('id', 1).single()
    if (data) {
  ...
  async function savePolicy() {
    ...
    const { error } = await supabase.from('club_policy').update({ content: draft, updated_at: now }).eq('id', 1)
    if (!error) {
      setContent(draft)
      setUpdatedAt(now)
      setEditing(false)
      ...
      {editing ? (
        <>
          ...
          <div className="modal-actions" style={{ marginTop: 10 }}>
            ...
            <button className="btn primary" disabled={saving} onClick={savePolicy}>
              {saving ? '저장 중...' : '저장'}
            </button>
          </div>
        </>
        ...
        <div className="policy-view">
          {content ? <pre className="policy-content">{content}</pre> : <p className="intro-placeholder">아직 등록된 정책이 없어요.</p>}
        </div>
      )}
